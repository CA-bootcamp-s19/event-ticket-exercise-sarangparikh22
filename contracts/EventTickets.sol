...
contract EventTickets {
    ...
    */
    constructor(string memory _desc, string memory _website, uint _totalTickets) public {
        owner = msg.sender;
    ...
    */
    function getBuyerTicketCount(address _addr) public view returns (uint) {
    ...
    */
    function buyTickets(uint _tic) public payable {
        require(myEvent.isOpen == true);
        require(myEvent.totalTickets >= myEvent.sales + _tic);
        require(msg.value >= TICKET_PRICE * _tic);

        myEvent.buyers[msg.sender] += _tic;
        myEvent.sales += _tic;
        msg.sender.transfer(msg.value - (TICKET_PRICE * _tic));
        emit LogBuyTickets(msg.sender, _tic);
    }
    /*
